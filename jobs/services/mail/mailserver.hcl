...
# TODO: Change this at some point plz
TLS_LEVEL=modern
...
# This module can further assist in avoiding spam emails by greylistinbrickie
...
POSTFIX_MAILBOX_SIZE_LIMIT=2147483648 # 2GB
...
LDAP_SERVER_HOST=ldap://{{ range service "openldap-ldap" }}{{ .Address }}:{{ .Port }}{{ end }}

# empty => ou=people,dc=domain,dc=com
# => e.g. LDAP_SEARCH_BASE=dc=mydomain,dc=local
LDAP_SEARCH_BASE="o=redbrick,dc=redbrick,dc=dcu,dc=ie"

# empty => cn=admin,dc=domain,dc=com
# => take a look at examples of SASL_LDAP_BIND_DN
LDAP_BIND_DN={{ key "mail/ldap/bind" }}

# empty** => admin
# => Specify the password to bind against ldap
LDAP_BIND_PW={{ key "mail/ldap/password" }}

# e.g. `"(&(mail=%s)(mailEnabled=TRUE))"`
# => Specify how ldap should be asked for users
LDAP_QUERY_FILTER_USER="(|(uid=%s)(mail=%s))"

# e.g. `"(&(mailGroupMember=%s)(mailEnabled=TRUE))"`
# => Specify how ldap should be asked for groups
# NOTE: Change this at some point plz TODO
LDAP_QUERY_FILTER_GROUP="(&(objectClass=brickie)(mail=%s))"
...
DOVECOT_USER_FILTER="(|(mail=%u)(mail=%n)(uid=%n))"
...
# took out the HomeDirectory option for now as its not used, but we can set it up later if needed
DOVECOT_USER_ATTRS=uidNumber=uid,gidNumber=gid,=home=/var/mail/%d/%n/home

DOVECOT_PASS_FILTER="(|(mail=%u)(mail=%n)(uid=%n))"
