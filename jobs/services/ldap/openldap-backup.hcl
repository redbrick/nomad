...
file=/storage/backups/nomad/${job_name}/${job_name}-$(date +%Y-%m-%d_%H-%M-%S).ldif

mkdir -p /storage/backups/nomad/${job_name}

alloc_id=$(nomad job status openldap | grep running | tail -n 1 | cut -d " " -f 1)

nomad alloc exec -task=openldap $alloc_id slapcat -F /bitnami/openldap/slapd.d -n 2 > "${file}"

find /storage/backups/nomad/${job_name}/ -name "${job_name}*" -ctime +3 -exec rm {} \; || true

if [ -s "$file" ]; then # check if file exists and is not empty
  echo "Backup successful"
  exit 0
else
  # Check if file exists before trying to remove it to avoid secondary error spam
  [ -f "$file" ] && rm "$file"
